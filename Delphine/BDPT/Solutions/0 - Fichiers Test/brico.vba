...
Sub new_code_suivi(code As Long, ligne_modif_usf As Long, file_suivi As String)
    ...
    If Application.WorksheetFunction.CountIf(Workbooks(base_data).Sheets(onglet_suivi).Range("C:C"), code) > 0 Then 'si le code est bien présent dans la sheet
        Dim num_code_suivi As Long
        num_code_suivi = Workbooks(base_data).Sheets(onglet_suivi).Range("C:C").Find(What:=code).Row ' récup de la ligne ou y a le code
    ...
    'Ici on a la ligne ou faut remppalcer le code adns base data dans l'onglet suivi-dlphine
    ...
    ' ici on a la ligne ou faut remplacer dans l'onglet fournisseur
...
suivi = fichier_suivi
        ...
        Else
            ...
            For j = 0 To ListBox1.ListCount - 1 'on itère jusqu'a ce qu'on tombe sur al li_gne selectionnée
                If ListBox1.Selected(j) = True Then
                        ...
                        'cas code flag siege
                        If Workbooks(workbook_compil).Sheets(onglet_compil).Cells(ligne_modif, newrange) = 1 Then 'Ca marché plus parcequ'il ont rajouté des colonnes
                            Dim choix
                                'boite confirmation changement si truc du siege
